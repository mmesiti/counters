...
_FD(vector_norm_flops , complex_norm_flops()*NF+real_op_flop()*NF); // + NF sums (not NF-1)
...
_FD(spinor_mul_add_assign_flops , spinor_mul_flops() + spinor_sum_flops());

// spinor field related flop count, for a single site
_FD(site_spinor_field_sqnorm_f_flops , 1.0/2 * spinor_norm_flops()); // mult + add
_FD(site_spinor_field_mul_add_assign_f_flops , 1.0/2 * spinor_mul_add_assign_flops());
...
_FD( site_Cphi_inv_flops , 1.0/2*(forward_substitution_loop_flop() + \
                                      backward_substitution_loop_flop()));

_FD( site_Cphi_inv_assign_flops , 1.0/2*(forward_substitution_loop_flop() + \
                                             backward_substitution_loop_flop() + \
                                             spinor_sum_flops()));
